...
app.get('/api/admin/init-db', async (req, res) => {
    try {
        ...
        // Index
        await query(`CREATE INDEX IF NOT EXISTS idx_chatbots_email ON chatbots(contact_email);`);
        ...
        res.status(500).json({ error: 'Failed to init db', details: error.message });
    }
});
